...
class IntakeRequest(BaseModel):
    doc_type: Literal["nda", "msa", "sow", "sla"]
    description: Optional[str] = Field(default=None, max_length=5000)
    provided_fields: dict = Field(default={})
...
{{
    ...
    "key_information": {{
        ...
        "start_date": "contract start/effective date in YYYY-MM-DD format, or null if not found",
        "end_date": "contract end/expiry date in YYYY-MM-DD format, or null if not found",
        "contract_value": "total contract value as a number (digits only, no currency symbol), or null if not found",
        "contract_type": "one of: nda, service, employment, lease, purchase, partnership, licensing, other",
        "duration": "contract duration as described in the document",
...
{body.text}"""
        ...
        messages = [
            {"role": "system", "content": "You are a legal AI analyst for a CLM platform. Base your analysis ONLY on the contract text provided. Do not infer or include information not present in the document. Provide precise, structured contract analysis."},
...
def chat(request: Request, body: ChatRequest):
    ...
    try:
        ...
        # Only search the knowledge base when no contract text is provided.
        # When a contract is attached, the answer must come from the document —
        # injecting KB results risks polluting the response with unrelated clauses.
        kb_context = ""
        question_stripped = body.question.strip()
        has_contract = bool(body.contract_text and body.contract_text.strip())
        needs_kb_search = not has_contract and len(question_stripped) >= MIN_QUERY_LENGTH_FOR_KB
        ...
        # ── Intent detection ────────────────────────────────────────────────
        q_lower = question_stripped.lower()
        is_risk_question = any(w in q_lower for w in [
            "risk", "risks", "risky", "danger", "dangerous", "concern", "concerns",
            "issue", "issues", "problem", "problems", "liability", "liabilities",
            "vulnerability", "vulnerabilities", "weakness", "weaknesses", "threat",
        ])
        ...
        if has_contract and is_risk_question:
            system_content = (
                "You are a senior legal AI analyst for a Contract Lifecycle Management system. "
                "A contract has been provided. Analyse it ONLY using the information in that contract — "
                "do not introduce facts or clauses not present in the document.\n\n"
                "When asked about risks, you MUST respond using EXACTLY this structure "
                "(use markdown, keep sections clearly separated):\n\n"
                "---\n"
                "## ⚠️ Contract Risk Assessment\n\n"
                "### 📊 Risk Summary\n"
                "One sentence overall risk verdict for this contract.\n"
                "**Overall Risk Score: X / 10**\n\n"
                "---\n\n"
                "### 🔢 Ranked Risks\n\n"
                "List every identified risk ranked highest to lowest. For each use EXACTLY:\n\n"
                "**#1 · Risk Title**\n"
                "🔴 Severity: High &nbsp;|&nbsp; Risk Score: 8/10\n"
                "> One concise sentence describing the risk and its potential impact.\n\n"
                "**#2 · Risk Title**\n"
                "🟠 Severity: Medium &nbsp;|&nbsp; Risk Score: 5/10\n"
                "> One concise sentence describing the risk and its potential impact.\n\n"
                "Use 🔴 for High (7–10), 🟠 for Medium (4–6), 🟢 for Low (1–3).\n"
                "Number them sequentially. Score each risk out of 10.\n\n"
                "---\n\n"
                "### ⚖️ Legal Analysis\n\n"
                "For each ranked risk, use EXACTLY:\n\n"
                "**#1 · Risk Title**\n"
                "**Relevant Clause:** Quote or reference the exact clause from the contract.\n"
                "**Legal Implication:** Explain in formal legal terms what this clause means "
                "and the legal exposure it creates for each party.\n\n"
                "---\n\n"
                "### ✅ Recommendations\n\n"
                "For each ranked risk, use EXACTLY:\n\n"
                "**#1 · Risk Title**\n"
                "**Action:** State the recommended contract amendment or negotiation point.\n"
                "**Proposed Language:** Suggest specific revised wording where possible.\n"
            )
        elif has_contract:
            system_content = (
                "You are a legal AI assistant for a Contract Lifecycle Management system. "
                "A contract document has been provided. Answer the question using ONLY the "
                "information found in that contract. Do not introduce facts, clauses, or terms "
                "not present in the document. If the answer is not in the contract, say so. "
                "You have access to the conversation history for continuity."
            )
        else:
            system_content = (
                "You are a legal AI assistant for a Contract Lifecycle Management system. "
                "Answer questions clearly and professionally. Use the provided knowledge base "
...
@app.post("/intake", response_model=IntakeResponse, tags=["documents"])
@limiter.limit("20/minute")
def intake(request: Request, body: IntakeRequest):
    """Smart intake: identify which template fields are still missing."""
    logger.info(f"POST /intake doc_type={body.doc_type} desc_len={len(body.description or '')}")

    import json

    templates_dir = Path(__file__).parent / "templates"
    template_path = templates_dir / f"{body.doc_type}.txt"
    if not template_path.exists():
        raise HTTPException(status_code=404, detail=f"Template not found: {body.doc_type}")

    template_text = template_path.read_text()
    all_fields = re.findall(r'\{([^}]+)\}', template_text)
    ...
    if body.description and len(body.description) >= 20:
        gemini_ready = getattr(request.app.state, "gemini_ready", False)
        if not gemini_ready:
            logger.warning("Gemini not ready — skipping field inference for intake")
        else:
            try:
                from tools import generate_text
                fields_list = ", ".join(required_fields)
                prompt = (
                    "Extract values for these contract fields from the description below. "
                    "Only include fields where the description gives a clear, unambiguous value. "
                    "Return ONLY a valid JSON object mapping field name to value (string). "
                    "If no fields can be extracted, return {}.\n\n"
                    f"Fields: {fields_list}\n"
                    f"Description: {body.description}"
                )
                messages = [{"role": "user", "content": prompt}]
                raw = generate_text(messages, max_tokens=400, temperature=0.1, task="intake")
                try:
                    inferred_fields = json.loads(_extract_json_from_response(raw))
                    if not isinstance(inferred_fields, dict):
                        inferred_fields = {}
                except (json.JSONDecodeError, ValueError):
                    logger.warning("intake: could not parse Gemini JSON response — treating as empty")
                    inferred_fields = {}
            except Exception as e:
                logger.warning("intake: Gemini inference failed: %s", e)
                inferred_fields = {}

    excluded = set(inferred_fields.keys()) | set(body.provided_fields.keys())
    pending_fields = [f for f in required_fields if f not in excluded]
    ...
    return IntakeResponse(
        ...
        ready_to_generate=(len(pending_fields) == 0),
        suggested_extras=[],
    )
...
def _embed_and_index(chunks: list[str], file_name: str) -> int:
    ...
    try:
        resp = es.search(
            ...
            query={"bool": {"filter": [
                ...
                {"term": {"document_hash": content_hash}},
            ]}},
            size=1,
            _source=False,
        )
    ...
    try:
        es.delete_by_query(
            index=INDEX_NAME,
            query={"term": {"metadata.file_name": file_name}},
            refresh=True,
        )
    except Exception:
        pass
        ...
        for offset, (chunk, emb_obj) in enumerate(zip(batch, result.embeddings)):
            actions.append({
                ...
                "_source": {
                    ...
                    "uploader_id":    "chat_upload",
                    "version":        1,
                    "risk_score":     None,
                    "actual_content": chunk,
                    "content_vector": emb_obj.values,
                    "metadata": {
                        "upload_date": _dt.utcnow().isoformat(),
                        "file_name":   file_name,
                        "is_latest":   True,
                        "doc_type":    "upload",
