...
_BOILERPLATE_PATTERNS = [
    ...
    re.compile(r"^\s*\d+\s*$"),
    re.compile(r"^[\s\-_=*]{10,}$"),
    re.compile(r"^\s*(confidential|proprietary|draft|internal use only)\s*$", re.IGNORECASE),
]

_BOILERPLATE_KEYWORDS = {
    "all rights reserved", "printed name", "signature below",
    "intentionally left blank", "end of document", "table of contents",
    "page break",
}


def _is_heuristic_boilerplate(chunk: str) -> bool:
    ...
    if len(stripped) < 150:
        for kw in _BOILERPLATE_KEYWORDS:
            if kw in lower and len(lower) < len(kw) + 40:
                return True
...
_RELEVANCE_PROMPT = """\
You are a contract document analyst. For each chunk below, decide if it contains \
substantive legal or business content worth keeping in a knowledge base.

KEEP: clauses, obligations, terms, definitions, conditions, rights, warranties, \
indemnities, payment terms, SLAs, penalties, governing law, dispute resolution, \
renewal/termination conditions, data protection terms, any specific business logic.

DISCARD: boilerplate headers/footers, tables of contents, signature blocks with \
no substantive terms, page numbers, formatting artifacts, repeated disclaimers \
with no unique content, blank fillers, cover pages with only titles/dates.

Return a JSON array of booleans, one per chunk. true = keep, false = discard.
...
def _filter_chunks_heuristic(chunks: list[str]) -> list[str]:
    kept = [c for c in chunks if not _is_heuristic_boilerplate(c)]
    dropped = len(chunks) - len(kept)
    if dropped:
        logger.info("Heuristic filter: dropped %d/%d chunks", dropped, len(chunks))
    return kept


def _filter_chunks_llm(chunks: list[str], client: genai.Client) -> list[str]:
    if not chunks:
        return chunks

    kept = []
    for batch_start in range(0, len(chunks), FILTER_BATCH_SIZE):
        batch = chunks[batch_start: batch_start + FILTER_BATCH_SIZE]
        numbered = "\n\n".join(
            f"--- Chunk {i+1} ---\n{c}" for i, c in enumerate(batch)
        )
        prompt = _RELEVANCE_PROMPT + numbered
        ...
        try:
            ...
            if isinstance(verdicts, list) and len(verdicts) == len(batch):
                for chunk, keep in zip(batch, verdicts):
                    if keep:
                        kept.append(chunk)
            else:
                logger.warning("LLM filter returned unexpected shape, keeping batch")
                kept.extend(batch)
        except Exception as e:
            logger.warning("LLM filter failed (%s), keeping batch as-is", e)
            kept.extend(batch)

    dropped = len(chunks) - len(kept)
    if dropped:
        logger.info("LLM relevance filter: dropped %d/%d chunks", dropped, len(chunks))
    return kept
    ...
    try:
        resp = es.search(
            ...
            query={
                "bool": {
                    "filter": [
                        {"term": {"metadata.file_name": source_name}},
...
def _delete_old_chunks(source_name: str):
    ...
    try:
        es.delete_by_query(
            index=INDEX_NAME,
            query={"term": {"metadata.file_name": source_name}},
            refresh=True,
        )
...
def ingest_document(file_path: str, doc_type: str = "SLA", customer: str = "Unknown"):
    ...
    logger.info("Chunks before filtering: %d", len(chunks))

    # 3b. Two-pass relevance filtering
    chunks = _filter_chunks_heuristic(chunks)
    chunks = _filter_chunks_llm(chunks, client)
        ...
        for offset, (chunk, embedding_obj) in enumerate(zip(batch, result.embeddings)):
            ...
            actions.append({
                ...
                "_source": {
                    ...
                    "uploader_id":    customer,
                    "version":        1,
                    "risk_score":     None,
                    "actual_content": chunk,
                    "content_vector": embedding_obj.values,
                    "metadata": {
                        "upload_date": __import__("datetime").datetime.utcnow().isoformat(),
